'use client'

import { motion } from 'framer-motion'
import Link from 'next/link'
import { portfolioData } from '@/lib/constants'
import { fadeUpVariants, containerVariants, itemVariants } from '@/lib/animations'

const Footer = () => {
  const { personalInfo, social, navigation } = portfolioData
  const currentYear = new Date().getFullYear()

  return (
    <footer className="border-t border-accent-green/10 bg-dark-900/50 backdrop-blur-md">
      <motion.div
        className="section-container"
        initial="hidden"
        whileInView="visible"
        viewport={{ once: true }}
        variants={containerVariants}
      >
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8 mb-8">
          {/* Brand */}
          <motion.div variants={itemVariants}>
            <Link href="#home" className="text-2xl font-grotesk font-bold mb-2">
              <span className="text-accent-green">VG</span>
              <span className="text-text-primary">.</span>
            </Link>
            <p className="text-text-secondary text-sm">{personalInfo.title}</p>
          </motion.div>

          {/* Quick Links */}
          <motion.div variants={itemVariants}>
            <h4 className="text-text-primary font-semibold mb-4">Navigation</h4>
            <ul className="space-y-2">
              {navigation.map((item) => (
                <li key={item.href}>
                  <Link
                    href={item.href}
                    className="text-text-secondary text-sm hover:text-accent-green transition-colors"
                  >
                    {item.label}
                  </Link>
                </li>
              ))}
            </ul>
          </motion.div>

          {/* Services */}
          <motion.div variants={itemVariants}>
            <h4 className="text-text-primary font-semibold mb-4">Services</h4>
            <ul className="space-y-2">
              <li>
                <span className="text-text-secondary text-sm hover:text-accent-green cursor-default">
                  Full Stack Development
                </span>
              </li>
              <li>
                <span className="text-text-secondary text-sm hover:text-accent-green cursor-default">
                  API Development
                </span>
              </li>
            </ul>
          </motion.div>

          {/* Connect */}
          <motion.div variants={itemVariants}>
            <h4 className="text-text-primary font-semibold mb-4">Connect</h4>
            <div className="flex gap-4">
              <motion.a
                href={social.github}
                target="_blank"
                rel="noopener noreferrer"
                className="w-10 h-10 flex items-center justify-center bg-dark-700 rounded-lg hover:bg-accent-green/20 transition-colors"
                whileHover={{ scale: 1.1 }}
              >
                <svg className="w-5 h-5 text-text-primary" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z" />
                </svg>
              </motion.a>
              <motion.a
                href={social.linkedin}
                target="_blank"
                rel="noopener noreferrer"
                className="w-10 h-10 flex items-center justify-center bg-dark-700 rounded-lg hover:bg-accent-green/20 transition-colors"
                whileHover={{ scale: 1.1 }}
              >
                <svg className="w-5 h-5 text-text-primary" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.475-2.236-1.986-2.236-1.081 0-1.722.728-2.004 1.431-.103.25-.129.599-.129.948v5.426h-3.555V9h3.555v1.346h.05c.504-.954 1.738-1.954 3.588-1.954 3.117 0 5.515 2.041 5.515 6.441v5.619zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.225 0z" />
                </svg>
              </motion.a>
            </div>
          </motion.div>
        </div>

        {/* Bottom Section */}
        <motion.div
          className="border-t border-accent-green/10 pt-8 flex flex-col sm:flex-row justify-between items-center gap-4"
          variants={fadeUpVariants}
        >
          <p className="text-text-secondary text-sm">
            © {currentYear} Vishal Gupta. All rights reserved.
          </p>
          <p className="text-text-secondary text-sm">
            Crafted with <span className="text-accent-green">❤️</span> using React, Next.js & Tailwind CSS
          </p>
        </motion.div>
      </motion.div>
    </footer>
  )
}

export default Footer
